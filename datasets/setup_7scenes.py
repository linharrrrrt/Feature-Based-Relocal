...
for ds in ['chess']:
	...
	def link_frames(split_file, variant):
		...
		mkdir(target_folder + variant + '/depth/')
		...
		for seq in split:
			...
			for img in images:
				print()
				os.system('copy ..\\..\\..\\'+src_folder+'\\'+ds+'\\'+seq+'\\'+img+ ' ' +target_folder+variant+'\\rgb\\'+seq+'-'+img)

			depths = [f for f in files if f.endswith('depth.png')]
			for img in images:
				os.system('copy ..\\..\\..\\'+src_folder+'\\'+ds+'\\'+seq+'\\'+img+ ' ' +target_folder+variant+'\\depth\\'+seq+'-'+img)

			# link folders
			poses = [f for f in files if f.endswith('pose.txt')]
			for pose in poses:
				os.system('copy ..\\..\\..\\'+src_folder+'\\'+ds+'\\'+seq+'\\'+pose+ ' ' +target_folder+variant+'\\poses\\'+seq+'-'+pose)
